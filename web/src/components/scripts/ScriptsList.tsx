...
const ScriptsList: React.FC<Props> = ({ onSelect, header, title, selectedId }) => {
  ...
  const visibleScripts = useMemo(() => {
    ...
    return Object.entries(scripts)
      .filter(([id, script]) =>
        (!state.search || getName(id).includes(state.search.toLowerCase())) &&
        (!state.tag || getTags(script)?.includes(state.tag) ||
          (state.tag === ScriptTag.Full && !getTags(script)?.includes(ScriptTag.Teen)) ||
          (state.tag === ScriptTag.Homebrew && !getTags(script)?.includes(ScriptTag.Official)
            && !getTags(script)?.includes(ScriptTag.WorldCup))))
      .sort(([id1], [id2]) => getName(id1).localeCompare(getName(id2)))
  }, [scripts, state, t])

  const getIcon = (script: Script) => {
    const tag = getScriptMeta(script).tags
      ?.find(tag => Object.values(ScriptTag).includes(tag as ScriptTag))
    return ScriptTagIcon[tag as ScriptTag] ?? ScriptTagIcon[ScriptTag.Full]
  }
    ...
    <IonItemSliding key={id}>
      <IonItem button detail={false} onClick={() => onSelect(id)}>
        {selectedId !== undefined &&
          <IonCheckbox slot='start' checked={selectedId === id} />}
        <IonIcon slot='start' icon={getIcon(script)} color='medium' />
        <IonLabel className='ion-text-nowrap'>
          <h2><Translation path={id} /></h2>
          <p className='ion-hide-sm-down'>{getScriptMeta(script)?.author}</p>
        </IonLabel>
      </IonItem>
